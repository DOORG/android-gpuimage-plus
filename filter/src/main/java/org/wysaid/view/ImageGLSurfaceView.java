...
public class ImageGLSurfaceView extends GLSurfaceView implements Renderer {
  ...
  public void setImageFile(File imageFile) {
    rx.Observable.just(imageFile).subscribeOn(Schedulers.io()).flatMap(file -> {
      try {
        Bitmap bmp = BitmapFactory.decodeStream(
            mContext.getContentResolver().openInputStream(Uri.fromFile(file)));
        int w = bmp.getWidth();
        int h = bmp.getHeight();
        ...
        if (s > 1.0f) {
          w /= s;
          h /= s;
          return rx.Observable.just(Bitmap.createScaledBitmap(bmp, w, h, false));
        } else {
          return rx.Observable.just(bmp);
        }
      } catch (FileNotFoundException e) {
        e.printStackTrace();
        return rx.Observable.error(e);
      }
    }).observeOn(AndroidSchedulers.mainThread()).subscribe(this::setImageBitmap, throwable -> {
      Log.e("ImageGLSurfaceView", throwable.toString());
    });
  }
  ...
  @Override public void onDrawFrame(GL10 gl) {
    ...
    GLES20.glViewport(mRenderViewport.x, mRenderViewport.y, mRenderViewport.width,
        mRenderViewport.height);
