 ...
 */
public class SimplePlayerGLSurfaceView extends GLSurfaceView
  ...
  public void setFitFullView(boolean fit) {
    mFitFullView = fit;
    if (mDrawer != null) calcViewport();
  }

  public boolean getFitFullView() {
    return mFitFullView;
  }
  ...
  public synchronized void setVideoUri(final Uri uri, final PlayPreparedCallback preparedCallback,
      final PlayCompletionCallback completionCallback) {
  ...
  public synchronized void setMaskBitmap(final Bitmap bmp, final boolean shouldRecycle,
      final SetMaskBitmapCallback callback) {
  ...
  @Override public void onFrameAvailable(SurfaceTexture surfaceTexture) {
    ...
    if (mLastTimestamp2 == 0) mLastTimestamp2 = System.currentTimeMillis();
  ...
  private void calcViewport() {
    ...
    Log.i(LOG_TAG, String.format("View port: %d, %d, %d, %d", mRenderViewport.x, mRenderViewport.y,
        mRenderViewport.width, mRenderViewport.height));
  }
  ...
  private void _useUri() {
    ...
    try {
      ...
      if (mPlayCompletionCallback != null) {
        this.post(new Runnable() {
          @Override public void run() {
            if (mPlayCompletionCallback != null) {
              if (!mPlayCompletionCallback.playFailed(mPlayer, MediaPlayer.MEDIA_ERROR_UNKNOWN,
    ...
    try {
      ...
      Log.i(LOG_TAG,
          String.format("Error handled: %s, play failure handler would be called!", e.toString()));
      if (mPlayCompletionCallback != null) {
        this.post(new Runnable() {
          @Override public void run() {
            if (mPlayCompletionCallback != null) {
  ...
  public synchronized void takeShot(final TakeShotCallback callback) {
    ...
    queueEvent(new Runnable() {
      @Override public void run() {
        ...
        GLES20.glReadPixels(mRenderViewport.x, mRenderViewport.y, mRenderViewport.width,
            mRenderViewport.height, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, buffer);
        Bitmap bmp = Bitmap.createBitmap(mRenderViewport.width, mRenderViewport.height,
            Bitmap.Config.ARGB_8888);
        bmp.copyPixelsFromBuffer(buffer);

        Bitmap bmp2 = Bitmap.createBitmap(mRenderViewport.width, mRenderViewport.height,
            Bitmap.Config.ARGB_8888);
